...
- As you see in the correlation figure, several variables are highly correlated (multicollinearity) to each other 
  ...
  models with all variables and removing insignificant variables, etc.
...
- The variable text_mean has an OR of 1.19 which suggests for one unit increase in text_mean we expect that about 1.19 times 
...
In the confusion matrix, diagonal numbers (75 and 30) indicates the correct predictions [true positives (TP) and true 
negatives (TN)] for the benign (0) and malignant (1) and  outcomes for test cancer datasets. The other numbers (11 and 27) 
...
- The fitted model has AUC 0.8315 suggesting better predictability in classification for breast cancer. 
