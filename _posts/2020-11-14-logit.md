...
- Logistic regression (logit model) explains the binary/dichotomous response variable (outcome as 0 and 1, yes and no, or 
  true and false) as a linear combinations of the single or multiple independent/predictor/explanatory variables 
  (discrete,  continuous, or categorical variables)
...
- The fitted model can be evaluated using the pseudo R-squared which measures improvement in model likelihood over the
  null model (unlike OLS R-squared which measures proportion of explained variance -- check here)
...
- In ROC, we can summarize the model predictability based on the area under curve (AUC). AUC range from 0.5 to 1 and
  model with higher the AUC has higher predictability. The fitted model has AUC 0.8315 suggesting the better 
  predictability. The points lying above the diagonal line represents better classification results.
The higher sensitivity and specificity reflects the well fitted model. In our case, 
...
## <span style="color:#33a8ff">Refrences</span>
