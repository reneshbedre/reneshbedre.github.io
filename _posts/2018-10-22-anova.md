...
In the histogram, the distribution looks approximately normal and suggetss that residuals are approximately
normally distributed
...
# if you have stacked table, you can use bioinfokit v1.0.3 or later for bartlett test
from bioinfokit.analys import stat 
res = stat()
...
```python
# if you have stacked table, you can use bioinfokit v1.0.3 or later for Levene test
from bioinfokit.analys import stat 
res = stat()
...
Similar to one-way ANOVA, you can use visual approaches, <b>Bartlett</b> or <b>Levene</b> and <b>Shapiro-Wilk test</b> 
...
plots and histogram. In residual plot, standardized residuals lie around the 45-degree line, it suggests that the 
residuals are approximately normally distributed. Besides, the histogram shows approximate normal distribution of 
residuals. 

<p style="background-color: #ddf3f5; padding: 20px;"><b>Note</b>:  The ANOVA model is remarkably robust to the violation
of normality assumption, which means that it will have non significant effect of Type I error rate and <i>p</i> values 
will remain reliable as long as there are no outliers</p> 
...
We will use levene test to check the assumption of homogeneity of variances,
...
As the p value (0.09) is non significant, we fail to reject null hypothesis and conclude that treatments have equal 
variances.
...
<span style="color:#9e9696"><i> Last updated: November 05, 2020</i> </span>
