...
We use ANOVA F test to check the 
...
| Source of <br> variation | degree of freedom (Df)  | Sum of squares (SS)   | Mean square (MS)  | F value | Significance |
|----|----|----|----|----|----|
| Factors (between)| Df | SS | MS | | |
| Residuals (within) | DF | SS | MS | | |
| Total | 29 | 33 |  | | |
...
<p align="center">
  ...
  \( SS_{T} = SS_F + SS_{E} \) 
   ...
   <p>
  Where,  \(  y_{ik}  \)  = \( k^{th} \) observation of \( i^{th}  \) level of factor,<br> 
          \(  \mu \)  = overall population mean (unknown) ,<br>
        \(  \alpha_i \)  = Main effect for factor (deviation from the \mu),<br>
         \(  \epsilon_{ij} \)  = Error,<br> 
         \( i \) = levels for factor (i = 1,2...,p) ,<br> 
         \( k \) = Observations or replicates for each factor (j = 1,2...,r) ,<br>
   </p>
   
| Source of <br> variation | degree of freedom <br>(Df)  | Sum of squares <br>(SS)   | Mean square <br>(MS)  | <i>F</i> value | Significance |
|----|----|----|----|----|----|
| Factor (between)| Df<sub>F</sub> = p-1 | SS<sub>F</sub> | MS<sub>F</sub> = SS<sub>F</sub> &#8725; Df<sub>F</sub> | MS<sub>F</sub> &#8725; MS<sub>E</sub> | |
...
| Total | Df<sub>T</sub> = pr-1 | SS<sub>T</sub> |  | | |
 ...
 <p>
  Where,  \(  SS_F    = \sum\limits_i p_i (\bar{y_i.} - \bar{y_..})^2 \),<br> 
         \(  SS_E    = \sum\limits_{ik} (y_{ik} - \bar{y_i.})^2 \),<br> 
       \(  SS_T    = SS_F + SS_E = \sum\limits_{ik} (y_{ik} - \bar{y_..})^2 \),<br> 
   </p>
...
<p align="center">
  ...
  \( SS_{T} = SS_A + SS_B + SS_{AB} + SS_{E} \) 
   ...
   <p>
  Where, \(  \alpha_i \)  = Main effect for factor A,<br>
...
<span style="color:#9e9696"><i> Last updated: November 20, 2020</i> </span>
