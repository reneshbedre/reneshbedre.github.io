...
- We will use `bioinfokit v0.8.6` or later
...
```python
>>> visuz.gene_exp.ma(df=df, lfc='log2FC', ct_count='value1', st_count='value2', color=('#00239CFF', 'grey', '#E10600FF'), 
    markerdot='*', figtype='svg', xlm=(0,16,1), ylm=(-6,6,1), axtickfontsize=10, axtickfontname='Verdana')
```
...
```python
>>> visuz.gene_exp.ma(df=df, lfc='log2FC', ct_count='value1', st_count='value2', color=('#00239CFF', 'grey', '#E10600FF'),
    legendpos='lower right', legendlabels=['Upregulated', 'Normal', 'Downregulated'])
```
