...
Let's visualize the interaction plot (also called profile plot) for interaction effects,

```python
from statsmodels.graphics.factorplots import interaction_plot
import matplotlib.pyplot as plt
fig = interaction_plot(d_melt['Genotype'], d_melt['years'], d_melt['value'], colors=['#4c061d','#d17a22', '#b4c292'])
plt.show()
```
...
- The interaction plot help to visualize the means of the response of the two groups. Generally, the X-axis should have 
  group with more levels. 
- From the interaction plot, the interaction effect is significant between the groups because three lines are not 
  parallel (roughly parallel group lines indicates no interaction). This interaction is also called ordinal interaction 
  as the lines do not cross to each other. 
- For more reliable conclusion of interaction plot, it should be verified with <i>F</i> test for interaction
